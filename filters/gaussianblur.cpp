...
GaussianBlur::GaussianBlur(const QImage &image, QProgressBar *progressBar) {
    ...
    _red = new int[static_cast<uint>(_width * _height)];
    _green = new int[static_cast<uint>(_width * _height)];
    _blue = new int[static_cast<uint>(_width * _height)];

    for (int j = 0; j < maxThreads; ++j)
        futures.addFuture(QtConcurrent::run([ = ] {
        for (int i = j *_height / maxThreads; i < (j + 1)*_height / maxThreads; ++i) {
            const QRgb *t = reinterpret_cast<const QRgb *>(image.scanLine(i));
...
QImage GaussianBlur::blur(int radius, qint8 **selectedTab) {
    ...
    newRed = new int[static_cast<uint>(_width * _height)];
    newGreen = new int[static_cast<uint>(_width * _height)];
    newBlue = new int[static_cast<uint>(_width * _height)];
...
int *GaussianBlur::boxesForGauss(double sigma, int n) {
    ...
    int *sizes = new int[static_cast<uint>(n)];
